...
func address(pk nostr.PubKey, d string) string {
	return fmt.Sprintf("%d:%s:%s", kindReview, pk.Hex(), d)
}
...
func TestDeletionMustNameTheSignersOwnReview(t *testing.T) {
...
func TestReviewOlderThanStoredDeletionIsRefused(t *testing.T) {
	sk := nostr.Generate()
	url := startRelay(t, sk.Public())
	c := connectAs(t, url, sk)

	mustPublish(t, c, deletion(t, sk, address(sk.Public(), "tmdb:movie:1"), nostr.Now()))
	ok, reason := c.publish(review(t, sk, "tmdb:movie:1", nostr.Now()-10))
	...
	if got := storedEvents(t, c, feedFilter(sk.Public())); len(got) != 0 {
		t.Errorf("refused review was stored: %v", got)
	}
}

func TestNewerReviewReplacesStoredDeletion(t *testing.T) {
	sk := nostr.Generate()
	url := startRelay(t, sk.Public())
	c := connectAs(t, url, sk)

	mustPublish(t, c, deletion(t, sk, address(sk.Public(), "tmdb:movie:1"), nostr.Now()-10))
	rec := review(t, sk, "tmdb:movie:1", nostr.Now())
	mustPublish(t, c, rec)

	if got := storedEvents(t, c, deletionsOf(sk.Public())); len(got) != 0 {
		t.Errorf("deletion still stored after a newer review: %v", got)
	}
	got := storedEvents(t, c, feedFilter(sk.Public()))
	if len(got) != 1 || got[0].ID != rec.ID {
		t.Errorf("reviews = %v, want exactly the new one", got)
	}
}

func TestDeletionOlderThanStoredReviewIsDiscarded(t *testing.T) {
	sk := nostr.Generate()
	url := startRelay(t, sk.Public())
	c := connectAs(t, url, sk)
	...
	got := storedEvents(t, c, feedFilter(sk.Public()))
	if len(got) != 1 || got[0].ID != rec.ID {
		t.Errorf("reviews = %v, want the newer review kept", got)
	}
	if got := storedEvents(t, c, deletionsOf(sk.Public())); len(got) != 0 {
...
func TestStoredReviewWinsATie(t *testing.T) {
	sk := nostr.Generate()
	url := startRelay(t, sk.Public())
	c := connectAs(t, url, sk)
	at := nostr.Now()

	first := review(t, sk, "tmdb:movie:1", at)
	mustPublish(t, c, first)
	// Same address and created_at, different content so the id differs.
	second := nostr.Event{Kind: kindReview, CreatedAt: at, Tags: nostr.Tags{{"d", "tmdb:movie:1"}}, Content: `{"title":"Other Placeholder"}`}
	...
	if len(got) != 1 || got[0].ID != first.ID {
		t.Errorf("reviews = %v, want the first one kept", got)
	}
}
...
func TestRequestLimitIsCappedAt500(t *testing.T) {
	...
	for i := range 520 {
		mustPublish(t, c, review(t, sk, fmt.Sprintf("tmdb:movie:%d", i), nostr.Now()))
	}
